...
(function () {
  ...
  /* ---------- 真实模式数据净化：清空全部演示假数据 ----------
   * 真实环境中未配置的表必须显示为空，绝不能残留演示数据误导运维。
   * 在确认后端存在（activateReal）与登录成功时各调用一次。 */
  var DEMO_LISTS = ['ifaceRates', 'hosts', 'hostProcesses', 'trafficLinks', 'arpBind', 'arpList', 'arpLogs',
    'speedLimitRules', 'bandwidthGuaranteeRules', 'appTypes', 'appLimitRules', 'accessRules',
    'connLimitRules', 'ddosRules', 'ipLibraries', 'policyRoutes', 'lineStatus', 'policyLogs',
    'portMapRules', 'portTriggerRules', 'domainRedirectRules', 'domainFilterRules',
    'loginBanList', 'dnsBlockLogs', 'secEvents', 'banList', 'dnsAuditLogs', 'staticRoutes',
    'sysLogs', 'attackLogs', 'ddosLogs', 'accessLogs', 'dhcpDetectLogs', 'subnets'];
  bridge.purgeDemoData = function () {
    DEMO_LISTS.forEach(function (k) { mock[k] = []; });
    mock.hostConnsMap = {};
    mock.pingResults = [];
    mock.currentConn = '0';
    mock.pppoe = '0';
    if (mock.ipLimit) mock.ipLimit.rules = [];
    /* 清空标量 demo 假默认值，防止真实数据到达前闪现（v1.1.7） */
    mock.runTime = ''; mock.cpu = ''; mock.cacheCapacity = ''; mock.currentCache = '';
    mock.skbNum = ''; mock.skbBufNum = ''; mock.connCapacity = '';
    mock.lanIp = ''; mock.lanMask = ''; mock.lanMac = ''; mock.lanMtu = '';
    mock.cpuModel = ''; mock.cpuCores = 0; mock.cpuFreq = '';
    mock.memory = { total: '', free: '' };
    /* 清空演示预置 WAN 线路（真实接口由 adoptRealData 依据 waninfo 重建） */
    mock.wanConfigs = [];
    mock.wanCount = 0;
  };
  ...
  function adoptRealData() {
    return fetchJSON(API + '?action=get&name=all', { headers: authHeaders() }).then(function (j) {
      ...
      var p = j.payload;
      if (p.sysinfo) {
        /* 真实模式：全部强制用后端返回值，绝不回退到 demo 假默认值（v1.1.7） */
        mock.runTime = p.sysinfo.runTime || '';
        mock.currentConn = String(p.sysinfo.currentConn != null ? p.sysinfo.currentConn : 0);
        mock.connCapacity = p.sysinfo.connCapacity || '';
        mock.cpuModel = p.sysinfo.cpuModel || '';
        mock.cpuCores = p.sysinfo.cpuCores || 0;
        mock.cpuFreq = p.sysinfo.cpuFreq || '';
        mock.sysVersion = p.sysinfo.release || '';
        if (p.sysinfo.hostname) mock.sysName = p.sysinfo.hostname;
        mock.goldenVersion = p.sysinfo.goldenVersion || mock.goldenVersion;
        /* 后端未提供 CPU 使用率/路由缓存/skb 计数器 → 显示占位，绝不显示 demo 假值 */
        mock.cpu = '';
        mock.cacheCapacity = '';
        mock.currentCache = '';
        mock.skbNum = '';
        mock.skbBufNum = '';
        mock.memory = { total: p.sysinfo.memTotalG || '', free: p.sysinfo.memFreeG || '' };
      }
      if (p.waninfo && p.waninfo.length) {
        ...
        p.waninfo.forEach(function (w, i) {
          ...
          if (t) {
            ...
            t.gateway = w.gateway || '';
            t.mac = w.mac || '';
            t.mtu = w.mtu ? parseInt(w.mtu, 10) : 0;
            t.dns1 = w.dns1 || '';
            t.dns2 = w.dns2 || '';
            t.status = w.status || '未连接';
          } else {
      ...
      if (p.laninfo) {
        mock.lanIp = p.laninfo.ip || '';
        mock.lanMask = p.laninfo.mask || '';
        mock.lanMac = p.laninfo.mac || '';
        mock.lanMtu = p.laninfo.mtu || '';
        if (p.laninfo.dhcpStart != null) mock.dhcp.start = p.laninfo.dhcpStart;
        if (p.laninfo.dhcpLimit != null) mock.dhcp.end = p.laninfo.dhcpLimit;
        if (p.laninfo.dhcpLease != null) mock.dhcp.lease = p.laninfo.dhcpLease;
      }
      if (p.hosts) {
      ...
      if (p.lbstatus) mock.lineStatus = p.lbstatus;
      ...
      /* 数据就绪回调：通知前端重渲染当前页（解决首屏用 demo 默认值、真数据回来不刷新） */
      if (bridge.onRealDataReady) { try { bridge.onRealDataReady(p); } catch (e) {} }
      return p;
    }).catch(function () { return null; });
  }
  /* 暴露给前端：保存后手动重拉真实数据并重渲染 */
  bridge.refresh = function () { return adoptRealData(); };
    ...
    return fetchJSON(API + '?action=auth&name=login', { method: 'POST', body: JSON.stringify({ user: user, pass: pass }) })
      .then(function (j) {
        if (j && j.ok && j.payload && j.payload.token) {
          ...
          bridge.purgeDemoData();
          /* 等真数据拉完再返回，保证 startApp 首次渲染就有真实数据 */
          return adoptRealData().then(function () {
  ...
  function activateReal() {
    bridge.mode = 'real';
    bridge.purgeDemoData();
    /* 真实模式：确保移除 kernel 注入的「演示模式」徽标（徽标可能在探测完成前后注入，带重试） */
    (function rmDemoBadge() {
      var el = document.getElementById('demoModeBadge');
      if (el) { el.remove(); return; }
      var tries = parseInt(rmDemoBadge._t || 0, 10) + 1;
      rmDemoBadge._t = tries;
      if (tries < 12) setTimeout(rmDemoBadge, 500);
    })();
  ...
  fetchJSON(API + '?action=auth&name=check', { headers: authHeaders() }).then(function (j) {
    if (j && j.ok) {
      ...
      /* 已登录的回头客：探测后立即拉真数据（adoptRealData 返回 promise，
         完成时触发 onRealDataReady → 前端重渲染，避免首屏空白/残留默认值） */
      if (authed) adoptRealData();
      if (bridge.onProbe) {
        bridge.onProbe({
          real: true,
          authed: authed,
