...
(function () {
  ...
  function persist() {
     ...
     * 安全门禁：真实模式仅允许「显式保存」触发的全量推送（showToast 包装），
     * 页面切换 / 关闭标签页（visibilitychange/beforeunload）绝不推送，
     * 避免把尚未回填的空规则推给后端清空真实配置。 */
    if (bridge.mode === 'real' && !bridge.__persistAllowed) return;
    bridge.__persistAllowed = false;
  ...
  window.showToast = function (msg) {
    if (msg && (msg.indexOf('已保存') >= 0 || msg.indexOf('已修改') >= 0 ||
      ...
      /* 真实模式：只有用户显式保存（toast 提示）才允许下一次全量推送 */
      if (bridge.mode === 'real') bridge.__persistAllowed = true;
